...
def species_export(filtered: bool = False) -> None:
    ...
    schema = pa.schema(
        [
            ...
            ("qualifiers", pa.string()),
            ("comments", pa.string()),
            ("study_id", pa.dictionary(pa.int32(), pa.string())),
        ]
    )
    sql = """
        SELECT
...
def main():
    ...
    if _summary_export:
        summary_export()
    if _bottle_export:
        bottle_export(filtered=args.filtered)
    if _mooring_export:
        mooring_export(filtered=args.filtered)
    if _ctd_export:
        ctd_export(filtered=args.filtered)
    if _species_export:
        species_export(filtered=args.filtered)
